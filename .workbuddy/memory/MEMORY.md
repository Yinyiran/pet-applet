...
- 弹窗/Sheet覆盖层：购买选项、同城详情、合作表单、课程详情、宠物表单、地址表单、订单详情、会员权益、充值、头像选择、售后表单、个人信息编辑、分佣概览、收益明细、提现、我的团队
...
- **入口**：profilePage 积分下方，菜单元 `.commission-menu-entry`（渐变背景突出显示），显示余额和等级标签
- **数据**：`js/commission.js` 中 `COMMISSION_CONFIG`（三级配置）、`commissionData`（统计）、`commissionLog`（明细）、`inviteList`（邀请列表）
- **弹窗**：概览(`#commissionOverlay`)、明细(`#commissionDetailOverlay`)、提现(`#withdrawOverlay`)、邀请(`#inviteOverlay`)
- **等级**：与 `userProfile.level` 联动（梵优合伙人/梵优主理人/分公司），自动计算返佣比例和升级进度
- **提现**：金额预设+手动输入、实时校验、最低¥10、选择微信/银行卡
